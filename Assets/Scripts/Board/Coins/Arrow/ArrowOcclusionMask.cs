...
{
    [Header("Mask Sprite (use soft-edged capsule)")]
    ...
    [Header("Independent stretch (world)")]
    ...
    [Header("Refresh")]
    [Min(0.02f)] public float reapplyInterval = 0.2f;
    ...
    void UpdateMaskTransform(CoinPlacementProbe probe)
    {
        ...
        float length = Mathf.Max(diameter, (dist + 2f * lengthPadding) * stretchY);

        if (_mask.sprite != capsuleSprite) _mask.sprite = capsuleSprite;
        if (_mask.sprite)
        {
            Vector2 sprSize = _mask.sprite.bounds.size;
    ...
    void ClearMaskOverrides()
    {
        if (_prevMask.Count == 0) return;
